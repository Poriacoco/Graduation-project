...
from address_data import load_data,scaler_train_data,scaler_test_data
...
X_list, Y_list,X_test_list, Y_test_list = load_data('/Users/poria/git/Graduation-project/LSTM/第五周/bio_all_34.xlsx')


#改进：在load_data中return四个数据，X_list,Y_list，X_test_list,Y_test_list
...
# 定义学习率调度器
scheduler = StepLR(optimizer, step_size=10, gamma=0.9)  # 每20个epoch学习率衰减为原来的0.9倍
...
for epoch in range(num_epochs):
    ...
    for inputs, lengths, targets in train_loader:
        ...
        loss = (criterion(outputs, target_sugar) * mask).sum() / mask.sum()
        optimizer.zero_grad()
        loss.backward()
    ...
    if epoch == 0 or loss_epoch < best_loss:
        torch.save(model.state_dict(), 'model_sugar.pt')

    # 加载并且评估
model_state_dict = torch.load('model_sugar.pt', map_location=device)
...
for i in range(1,2):  # 对于两个输出值
    true_values = targets_inverse[:, :, i][valid_mask]
    predicted_values = preds_inverse[:, :, 0][valid_mask]
