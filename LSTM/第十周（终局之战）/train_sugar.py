...
from model_sugar import train_model ,evaluate_model,LSTMModel
...
def cross_validation(full_data, n_splits=5):
    ...
    for fold, (train_idx, val_idx) in enumerate(kfold.split(full_data)):
        ...
        for df in train_data:
            ...
            X = np.hstack([
                ...
                rotation_speed,
                pressure_feature,
                fold_scaler_X.fit_transform(other_features)
            ])

            y2 = fold_scaler_Y2.fit_transform(df['残糖g/dl_next'].values.reshape(-1, 1))

            X_train.append(torch.FloatTensor(X))
            y2_train.append(torch.FloatTensor(y2))

            # 保存本fold的scaler
            joblib.dump(fold_scaler_X, f'scalers/scaler_X_fold{fold}.save')
            joblib.dump(fold_scaler_Y2, f'scalers/scaler_Y2_fold{fold}.save')
        ...
        for df in val_data:
            ...
            X = np.hstack([
                ...
                rotation_speed,
                pressure_feature,
                fold_scaler_X.transform(other_features)
            ])
            y2 = fold_scaler_Y2.transform(df['残糖g/dl_next'].values.reshape(-1, 1))
        ...
        train_dataset = TensorDataset(
            ...
            pad_sequence(y2_train, batch_first=True, padding_value=-1)
        )
        val_dataset = TensorDataset(
            pad_sequence(X_val, batch_first=True, padding_value=-1),
            torch.tensor([x.shape[0] for x in X_val]),
            pad_sequence(y2_val, batch_first=True, padding_value=-1)
        )
        # 创建DataLoader
        ...
        for epoch in range(num_epochs):
            ...
            if val_loss < best_val_loss:
                ...
                # 仅保存模型参数
                torch.save(model.state_dict(), f"models/sugar_fold{fold}.pt")
                no_improvement = 0
            else:
                no_improvement += 1
        ...
        # 最终评估
        model.load_state_dict(torch.load(f"models/sugar_fold{fold}.pt"))
        ...
        preds = fold_scaler_Y2.inverse_transform(preds.reshape(-1, 1))
        true = fold_scaler_Y2.inverse_transform(true.reshape(-1, 1))
...
if __name__ == "__main__":
    ...
    # 执行交叉验证
    results = cross_validation(full_data, n_splits=11)
