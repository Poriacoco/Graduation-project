    ...
    for fold, (train_idx, val_idx) in enumerate(kfold.split(full_data)):
        ...
        for epoch in range(num_epochs):
            ...
            if val_loss < best_val_loss:
                ...
                # 仅保存模型参数
                torch.save(model.state_dict(), f"sugar_fold{fold}.pt")
                no_improvement = 0
            else:
                no_improvement += 1
        ...
        # 最终评估
        model.load_state_dict(torch.load(f"sugar_fold{fold}.pt"))
        ...
        preds = scaler_Y2.inverse_transform(preds.reshape(-1, 1))
        true = scaler_Y2.inverse_transform(true.reshape(-1, 1))
...
if __name__ == "__main__":
    # 加载完整数据
    full_data = load_data('bio_all_34_new2.xlsx')
